...
// Supriya Naidu
// Lecture 6: Common errors with conditionals
...
int main()
{
    ...
    cheerios = 150;
    miniWheats = 75;
    // The ; below will generate a warning 
    // The cout statement will always print. This is because the ; is an "empty statement" and it effectively ends the if statement. The cout statement becomes part of "the rest of the program"
    ...
    // Error #7: validating user input - if the input value must meet certain conditions to be valid, then to spot the user error you need to check for the INVERSE of the given conditions
    ...
    // Error #8: always check if any value(s) can turn the condition true (or false). 
    // Particularly useful for variables that are allowed to have a range of values
    // using &&, ||

    // Error #9: forgetting to use "break;" in the switch statement case. The case will "fall through"
